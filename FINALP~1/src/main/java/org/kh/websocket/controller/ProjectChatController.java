package org.kh.websocket.controller;
...
public class ProjectChatController {
	...
	public String chat (Model model,HttpServletRequest req) throws Exception{
		...
		ChatMemberDto chatM = projectChatService.getRoomMember(new ChatMemberDto(0, login.getMemberId(), "",""));
		
		//만약 채팅방을 처음 입장하는 것이라면 방에대한 아이디정보를 생성
		if(chatM ==null) {
			projectChatService.addRoomMember(new ChatMemberDto(0, login.getMemberId(), "all", "all"));
			
			//추가를 한다음 chatM을 다시 받아오도록한다.
			chatM = projectChatService.getRoomMember(new ChatMemberDto(0, login.getMemberId(), "",""));
			logger.info("아이디 정보 추가 성공!" + new Date());
			
		}
		//존재한다면 방정보를 all로 변경 
		else {
			projectChatService.updateRoomMember(new ChatMemberDto(0,  login.getMemberId(), "all", "")); //room 변경
	...
	public String createChatRoom (Model model, HttpServletRequest req, ChatDto chat) throws Exception{
		...
		projectChatService.updateRoomMember(new ChatMemberDto(0, login.getMemberId(), chat.getName(), ""));
	...
	public String MoveChatRoom (Model model, HttpServletRequest req, String roomName) throws Exception{
		...
		//이동하게 될 방 이름으로 수정
		projectChatService.updateRoomMember(new ChatMemberDto(0, login.getMemberId(), roomName, ""));
