...
<body onload="loadJSON()" data-spy="scroll" data-target=".navbar-collapse">
    ...
    <div class="culmn">
        ...
        <section id="feature" class="feature p-top-50">
            <div class="container">
                <div style="width: 530px; margin:3% auto; text-align: center;">
                    <c:choose>
                        <c:when test="${not empty requestScope.member }">
                            <div style="font-size: 30px;">
                                ${requestScope.member.memberId }님의 비밀번호를 찾기위한 <br> 인증수단을 선택해 주세요<br>
                            </div>

                            <input type="hidden" name="idSearchMemberPhone" value="${requestScope.member.memberPhone}">
                            <input type="hidden" name="idSearchMemberEmail" value="${requestScope.member.memberEmail}">

                            <div style=" margin:3%; font-size: 18px;text-align: left;">
                                <label><input type="radio" onclick="idPhoneSearch()" name="search"><span id="phoneSpan"></span></label> <br>
                                <label><input type="radio" onclick="idEmailSearch()" name="search"> <span id="emailSpan"></span></label> <br>
                            </div>


                            <div id="idPhone">
                                <form action="/pwModifyFrm.do" id="idPhoneSearchFrm">

                                    <input type="text" name="memberName" class="form-control" style="height:40px;width: 300px; margin:auto;" placeholder="이름">
                                    <div class="input-group mb-3" style="width: 222px; margin-left: 21.8%; ">
                                        <input type="text" name="memberPhone" style="height:40px; margin-top: 5px;" class="form-control" onkeyup="fn_phoneRegCheck()" placeholder="핸드폰번호'-'포함">
                                        <div class="input-group-append" style="width: 0px;">
                                            <button type="button" class="btn" style="height: 40px;margin-top: 5px;" onclick="fn_phoneCheck()">인증번호</button><br>
                                        </div>
                                    </div>

                                    <span id="phoneRegCheckMsg"></span><br>
                                    <input type="text" id="phoneCheck" placeholder="인증번호 6자리 입력" class="form-control" style="height:40px; width: 300px;margin: auto;" onkeyup="fn_phoneNumberCheck()" disabled>
                                    <span id="phoneCheckSpan"></span><br>
                                    <input type="hidden" name="action" value="go">
                                    <input type="hidden" name="sphone1" value="010">
                                    <input type="hidden" name="sphone2" value="5388">
                                    <input type="hidden" name="sphone3" value="8918">
                                    <input type="button" class="btn" style="width: 300px; height:40px;" value="다음" onclick="fn_phoneFlagCheck()">
                                </form>
                            </div>


                            <div id="idEmail">
                                <form action="/pwModifyFrm.do" id="idEmailSearchFrm">


                                    <input type="text" name="memberName" class="form-control" placeholder="이름" style="height:40px;width: 300px; margin:auto;">
                                    <div class="input-group mb-3" style="width: 223px; margin-left: 21.8%;">
                                        <input type="text" name="memberEmail" class="form-control" style="height:40px; margin-top: 5px;" onkeyup="fn_emailRegCheck()" placeholder="이메일주소">
                                        <div class="input-group-append" style="width: 0px;">
                                            <button type="button" class="btn" onclick="fn_sendMail()" style="height: 40px;margin-top: 5px;">인증번호</button>
                                        </div>
                                    </div>

                                    <span id="emailRegCheckMsg"></span><br>

                                    <input type="text" id="emailCheck" class="form-control" style="width: 300px;margin: auto;  height:40px;" placeholder="인증번호 6자리 입력" onkeyup="fn_emailNumberCheck()" disabled>
                                    <span id="emailCheckSpan"></span><br>
                                    <input type="button" value="다음" class="btn" style="width: 300px; height:40px; " onclick="fn_emailFlagCheck()">
                                </form>
                            </div>
                        </c:when>
                        <c:otherwise>
                            <div style="width:500px;margin: auto; font-size: 30px; ">
                                ...
                                <div style="margin: auto; width:200px; ">
                                    <a href="/idSearchFrm.do" style="text-decoration: none;"><button type="button" class="btn" style="color:gray; width: 100%;text-decoration: none;">아이디찾기</button></a>
                ...
                <script>
                    ...
                    $(document).ready(function() {
                        var len = $("[name=idSearchMemberEmail]").val().split('@')[0].length - 3;
                        $("#phoneSpan").html(" 회원정보에 등록한 휴대전화로 인증 " + $("[name=idSearchMemberPhone]").val().replace(/^(\d{3})-?(\d{1,2})\d{3}-?(\d{1,2})\d{3}$/, '$1-$2***-$3***'));
                        $("#emailSpan").html("회원정보에 등록한 이메일로 인증 " + $("[name=idSearchMemberEmail]").val().replace(new RegExp('.(?=.{0,' + len + '}@)', 'g'), '*'));
                        $("#idEmail").hide();
                        $("#idPhone").hide();
                    });
